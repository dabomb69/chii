...
snd ("$servname VERSION :$servgecos)");
$time = time;
snd (":$sid UID $uid".time."$nick $host $host $ident $clientip $umodes :$clientgecos");
snd (":$sid$uid OPERTYPE $opertype");
snd ("ENDBURST");
snd ("PRIVMSG Connected to $server");
...
if($channel eq "$servicechan") {
	...
	if(($nickname =~ /jcs\.me\.uk/) || ($nickname =~ /spartairc\.co\.cc/)) {
		...
		if(substr($mtext, 0, 43) eq "\002CONNECT\002: Client connecting on port 6667: ") {
			...
			snd("PRIVMSG $connectednick :\001VERSION\001");
			...
			if($chcreatenick eq $adminnick) {
				...
				snd("MODE $createdchannel +qa $adminnick $adminnick");
...
if($echoon == 1) {
	if($nickname eq $usertoecho) {
		if(substr($mtext, 0, 7) eq "ACTION ") {
			snd("PRIVMSG $channel :\001$mtext\001");
		} else {
			sndtxt($mtext);
		}
		next;
...
if ($msgto eq $nickname && $ctcp_hax == 1 && $ctcp_reply == 1) {
  ...
  if($mtext =~ "^VERSION") {
  	if(exists($pendingversion{$nicktoversion})) {
  		...
  		snd("PRIVMSG $channelforversion :$nickname version reply: $tehversion");
...
if(lc($firstword) eq "ping") {
	...
	if($nicktoping eq "me") {
		...
		snd ("PRIVMSG $nicktoping :PING " . time . "");
		if ($notime == 1) {
...
if(lc($firstword) eq "ver") {
	...
	snd("PRIVMSG $nicktoversion :\001VERSION\001");
...
if (lc($firstword) eq "help") {
  ...
  if (index($text," ") == -1) { 
    snd("NOTICE $nickname :Avalible commands are: $cmdref");
    next;  
  }
  ...
  if (!defined($hlp{$query})) {
    snd ("NOTICE $nickname :No help is availibe for $query.");
    next;
  }

  snd ("NOTICE $nickname :$query - $hlp{$query}");
  snd ("NOTICE $nickname :$query - $xmp{$query}");
  snd ("NOTICE $nickname :$query - $amp{$query}");
  next;
...
if(lc($firstword) eq "join") {
	if($usermode =~ / ADMIN /) {
		...
		snd("SAMODE $channeltojoin +ao $botnick $botnick");
	} else {
		sndtxt("Access Denied.");
	}
	next;
