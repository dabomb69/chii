...
sub HUP_handler {
  ...
  snd (":$sid GLOBOPS :chii: Caught a SIGHUP, becoming a semi daemon.");
...
foreach $module(@modules) {
	print "${win321}module$module.pl";
	do "${win321}module$module.pl" ;
	if(($! or $@) ne "") {
		print " $mfail ($!)";
		die;
	} else {
...
#snd (":$sid UID $uuid2 1 micah chii. services micah 127.0.0.1 1 +iIkBwH :$clientgecos");
#snd (":$sid UID $uuid3 1 Hideki zelazny.freenode.net irc.freenode.net hideki 140.211.166.3 1 +iIB :$clientgecos");
#snd (":$uuid3 OPERTYPE $opertype");
#snd (":$uuid2 OPERTYPE $opertype");

snd ("ENDBURST");
snd ("PRIVMSG $ctrl :Connected to $server");
snd (":$sid FJOIN $ctrl ".time." + ,$uuid");
#snd (":$sid FJOIN $ctrl ".time." + :ao,$uuid");
#snd (":$sid FJOIN #lobby ".time." + :ao,$uuid");
...
if ($command eq "332") {
	if($topiccommand eq 1) {
		snd("PRIVMSG $topicchan :$topicnick, the topic for $topicchan is \"$mtext\"");
		$topiccommand = 0;
	}
}
...
foreach $badword(@badwords) {
	if($mtext =~ /$badword/) {
		snd("KILL $nickname :Don't use bad words in this network.");
	}
	next;
}
...
#if ($hostmask =~ $staff) {
#  $usermode = " ADDFACTS DELFACTS DELALLFACTS ADMIN OP AV STAFF";
##############
# LOOKUP AXS #
##############
...
if ((uc($command) ne 'QUIT') && (uc($command) ne 'PART')) {
  for ($i = 0;$i < ($#msg+1);$i++) {
    ...
    if (lc($nickname) eq lc($recipient)) {
      snd ("PRIVMSG $nickname :You have a tell: $message");
      #print "\$sender = $sender \n";
      snd("PRIVMSG $sender :$nickname has read your tell.");
      splice (@msg,$i,1);
      $i--;
    }
...
if ($msgto eq $nickname && $ctcp_hax == 1 && $ctcp_reply == 1) {
  if ($command eq "PRIVMSG") {
    if ($mtext =~ "^VERSION") {
      snd ("NOTICE $nickname :\001VERSION $bot_version_number by daBomb69 \001");
      if ($adminnick ne '') {
  ...
  if($mtext =~ "^VERSION") {
  	if(exists($pendingversion{$nicktoversion})) {
  		...
  		snd(":$uuid3 PRIVMSG $ctrl :$nickname version reply: $tehversion");
...
#  next;
#}
#if(lc($firstword) eq "ping") {
#	$nicktoping = substr($text, 5);
#	if($nicktoping eq "me") {
#		snd (":$uuid PRIVMSG $nickname :PING " . time . "");
#		if ($notime == 1) {
#			$pendingping{$nickname} = $msgto . "\001" . time();
#		} else {
#			$pendingping{$nickname} = $msgto . "\001" . Time::HiRes::time();
#		}
#	} else {
#		snd (":$uuid PRIVMSG $nicktoping :PING " . time . "");
##		if ($notime == 1) {
#			$pendingping{$nicktoping} = $msgto . "\001" . time();
#		} else {
#			$pendingping{$nicktoping} = $msgto . "\001" . Time::HiRes::time();
#		}
#	}
#	next;
#}
...
if (lc($firstword) eq "help") {
  ...
  local $query = "";
#	if($usermode !~ / STAFF /) {
#		snd("NOTICE $nickname :You are not authorized to perform this operation.");
#		next;
#	}
  if (index($text," ") == -1) { 
  ...
  snd (":$uuid NOTICE $nickname :$query - $hlp{$query}");
  snd (":$uuid NOTICE $nickname :$query - $xmp{$query}");
  snd (":$uuid NOTICE $nickname :$query - $amp{$query}");
  next;
...
if(lc($firstword) eq "ops") {
	...
	if($halp eq "") {
		foreach $op(@ops) {
	  		snd("NOTICE $op :$op, your assistance is requested in $channel by $nickname");
	  	}
	  	next;
	  } else {
	  	foreach $op(@ops) {
	  		snd("NOTICE $op :$op, assistance with \"$halp\" is requsted in $channel by $nickname");
	  	}
	  	next;
	  }
}
...
if (lc($text) eq "quit") {
  if ($usermode =~ / SUPERADMIN /) {
    ...
    print NOSPAWN "quit";
    close (NOSPAWN);
    snd ("QUIT :Quit command used by $nickname. $botname powered by $bot_version_number");
    sleep 1;
    &Cleanup;
    exit;
  } else {
...
if(lc($firstword) eq "join") {
	if($usermode =~ / ADMIN /) {
		...
		snd(":$uuid JOIN $channeltojoin");
		snd(":$uuid WHO $channeltojoin");
		snd("MODE $channeltojoin +ao $botnick $botnick");
	} else {
		sndtxt("Access Denied.");
	}
	next;
...
if (lc($text) eq "restart") {
  if ($usermode =~ / ADMIN /) {
    ...
    snd ("QUIT :Restart requested by $nickname. $botname powered by $bot_version_number.");
    sleep 1;
    &Cleanup;
    exit;
  }
...
####################
if ($line =~ / PING /) {
 #snd(":$sid PONG " . substr($line,index($line,":")+1));
 snd(":$sid PONG $sid 443");
 snd(":$uuid PRIVMSG #lobby :Pong! =D");
 snd(":$uuid PRIVMSG #lobby :Pong! =D");
 snd(":$uuid PRIVMSG #lobby :Pong! =D");
 snd(":$uuid PRIVMSG #lobby :Pong! =D");
 }
